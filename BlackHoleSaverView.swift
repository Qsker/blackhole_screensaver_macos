...
enum Presets {
    static let all: [Preset] = [
        ...
        Preset(name: "卡冈图雅",  look: DiskLook(
        ...
        Preset(name: "M87* 甜甜圈",  look: DiskLook(
        ...
        Preset(name: "正面余烬",  look: DiskLook(
...
final class BlackHoleSaverView: ScreenSaverView {
    ...
    override var hasConfigureSheet: Bool {
        try? "hasConfigureSheet called".write(toFile: "/tmp/bh_debug.log", atomically: true, encoding: .utf8)
        return true
    }

    override var configureSheet: NSWindow? {
        try? "configureSheet called".write(toFile: "/tmp/bh_debug.log", atomically: true, encoding: .utf8)
        if sheet == nil { sheet = buildSheet() }
        guard let sht = sheet else {
            try? "configureSheet: sheet is nil".write(toFile: "/tmp/bh_debug.log", atomically: true, encoding: .utf8)
            return nil
        }
        let s = Settings.load()
        ...
        NSLog("BlackHoleSaver: returning configureSheet frame=\(NSStringFromRect(sht.frame))")
        return sht
    }

    private func buildSheet() -> NSWindow {
        // Build controls
        let presets = NSPopUpButton(frame: NSRect(x: 0, y: 0, width: 240, height: 24), pullsDown: false)
        ...
        let quality = NSPopUpButton(frame: NSRect(x: 0, y: 0, width: 240, height: 24), pullsDown: false)
        quality.addItems(withTitles: ["自动（推荐）", "完整分辨率",
                                      "均衡（75%）", "性能（50%）"])
        qualityPopup = quality

        func label(_ s: String) -> NSTextField { NSTextField(labelWithString: s) }

        // Use a simple stack-based layout that's reliable on macOS 27
        ...
        var y: CGFloat = CGFloat(rows.count) * rowHeight + 60
        let contentWidth: CGFloat = 400
        let contentHeight: CGFloat = y + 10
        ...
        for (title, control) in rows {
            y -= rowHeight
            let lbl = label(title)
            lbl.frame = NSRect(x: 20, y: y, width: 100, height: 22)
            lbl.alignment = .right
            content.addSubview(lbl)
            control.frame = NSRect(x: 128, y: y - 2, width: 252, height: 24)
            content.addSubview(control)
        }

        // Buttons at the bottom
        let cancel = NSButton(title: "取消", target: self,
                              action: #selector(sheetCancel(_:)))
        cancel.keyEquivalent = "\u{1b}"
        cancel.frame = NSRect(x: contentWidth - 174, y: 10, width: 80, height: 28)
        let ok = NSButton(title: "确定", target: self, action: #selector(sheetOK(_:)))
        ok.keyEquivalent = "\r"
        ok.frame = NSRect(x: contentWidth - 90, y: 10, width: 76, height: 28)
        ...
        NSLog("BlackHoleSaver: built configureSheet")
        return window
    }
...
final class Renderer: NSObject, MTKViewDelegate, @unchecked Sendable {
    ...
    /// ScreenCaptureKit. On macOS 27 the saver runs inside
    /// WallpaperLegacyExtension (not legacyScreenSaver), so the Screen
    /// Recording permission must be granted to WallpaperAgent.app or
    /// WallpaperLegacyExtension.appex. Run fix-permissions.sh for help.
    /// Asynchronous: the texture appears a moment after the saver starts;
    /// the shader shows the starfield fallback until then.
    func captureScreen(behind window: NSWindow?) {
        // Snapshot values on the main actor before the unstructured Task.
        ...
        // On macOS 27 the saver host is WallpaperAgent; the legacy
        // screenSaverWindow level key may not match the actual layer.
        // We try the level-based exclusion but also fall back to a
        // full-display capture if that yields no windows to exclude.
        ...
        Task { [weak self] in
            ...
            do {
                ...
                // Exclude windows at or above the screensaver level so we
                // capture only the desktop beneath. On macOS 27 the saver
                // is composited by WallpaperAgent — its windows may have
                // a higher level or belong to a different process, so a
                // windowNumber match is also tried.
                var excluded = content.windows.filter {
                    $0.windowID == saverWindowID || $0.windowLayer >= saverLevel
                }
                // If the level-based filter excluded nothing (possible on
                // macOS 27 where WallpaperAgent manages the overlay),
                // fall back to capturing the full display. The first
                // Metal frame hasn't rendered yet so the saver overlay
                // should be transparent.
                if excluded.isEmpty {
                    excluded = content.windows.filter {
                        $0.windowID == saverWindowID
                    }
                }
                ...
                let loader = MTKTextureLoader(device: captureDevice)
                ...
                NSLog("BlackHoleSaver: ScreenCaptureKit capture failed: \(error)")
                NSLog("BlackHoleSaver: Grant Screen Recording permission to WallpaperAgent.app " +
                      "or WallpaperLegacyExtension.appex. See fix-permissions.sh for help.")
            }
        }
    }
